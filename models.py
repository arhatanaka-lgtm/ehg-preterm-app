...
FREQ_BANDS = {
    "B0 (0,05-1,0 Hz) Contracoes uterinas": 0.58,
    "B1 (1,0-2,2 Hz) Harmonicos baixos":    0.63,
    "B2 (2,2-3,5 Hz) Reverberacao cardiaca": 0.69,
    "B3 (3,5-5,0 Hz) Harmonicos altos":     0.72,
    "Full (0,05-4,0 Hz) Espectro completo":  0.74,
}
...
COMPARISON_DATA = [
    # (Tecnologia, Desfecho, Populacao, AUC_centro, AUC_min, AUC_max, Destaque)
    ("Informacoes clinicas",          "< 37 sem", "Gestacao unica, assintomaticas", 0.64, 0.61, 0.67, False),
    ("Modelo Clinico (Presente estudo)","< 37 sem","TPEHGDB+TPEHGTDS",             0.65, 0.63, 0.67, False),
    ("Fibronectina alfa",             "< 37 sem", "Assintomaticas",                0.65, 0.63, 0.66, False),
    ("Fibronectina alfa",             "< 37 sem", "Sintomaticas",                  0.71, 0.69, 0.73, False),
    ("Comprimento colo uterino",      "< 37 sem", "Nuliparas, assintomaticas",     0.67, 0.64, 0.70, False),
    ("Clinico + Placentario",         "< 37 sem", "Gestacao unica, assintomaticas",0.72, 0.66, 0.77, False),
    ("EHG (outros metodos)",          "< 37 sem", "TPEHGDB",                       0.61, None, None, False),
    ("EHG end-to-end DL",             "< 37 sem", "TPEHGDB",                       0.69, None, None, False),
    ("EHG (Presente estudo) *",       "< 37 sem", "TPEHGDB+TPEHGTDS",             0.74, 0.73, 0.76, True),
    ("Combinado EHG+Clinico (Presente estudo) **","< 37 sem","TPEHGDB+TPEHGTDS", 0.78, 0.76, 0.80, True),
    ("Fibronectina alfa",             "< 1 sem",  "Sintomaticas (imminente)",      0.84, 0.80, 0.87, False),
    ("Comprimento colo uterino",      "< 1 sem",  "Sintomaticas (imminente)",      0.84, None, None, False),
]

# ─── Variáveis clínicas com risco relativo ───────────────────────────────────
CLINICAL_VARS = [
    # (nome_exibicao, nome_interno, tipo, RR_centro, RR_min, RR_max)
    ("Paridade (multipara)", "parous",   "bin", 1.32, 0.79, 2.20),
    ("Aborto previo",        "abortion", "bin", 0.72, 0.23, 2.28),
...
def binormal_roc(auc: float, n_points: int = 200):
    """
    Gera curva ROC aproximada usando modelo binormal simetrico.
    TPR = Phi(a + b*Phi^-1(FPR)), onde a = sqrt(2) * Phi^-1(AUC), b = 1.
    """
...
def compute_risk_score(inputs: dict) -> dict:
    ...
    das variaveis binarias publicadas. Retorna score normalizado [0-1]
    e classificacao.
    NOTA: Os coeficientes beta do LASSO nao foram publicados.
    Escore e APROXIMADO e de uso exclusivamente educacional.
    """
    log_rr_sum = 0.0
